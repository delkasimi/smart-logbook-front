...
import SolutionDataTable from "../components/tickets/SolutionDataTable";
...
const SolutionsPage = () => {
  ...
  const [isSolutionDetailsModalOpen, setSolutionDetailsModalOpen] =
    useState(false);
  ...
  useEffect(() => {
    ...
    fetch(`${config.API_BASE_URL}/solution-data`)
      ...
      .then((data) => {
        ...
        const codeObjetOptions = Array.from(
          new Set(formattedData.map((item) => item.codeObjet))
        ).filter(Boolean);
        const issueOptions = Array.from(
          new Set(formattedData.map((item) => item.issue))
        ).filter(Boolean);

        // Update filterColumns with dynamic options
        setFilterColumns((oldColumns) =>
      ...
      <div class="row">
        <div className="col-12">
          <div className="card">
            ...
            <div class="card-body">
              <SolutionDataTable
                tabledata={filteredSolutions}
                handleViewDetails={handleviewSolutionDetails}
              />
            </div>
          </div>
        </div>
