...
const SolutionDetailsModal = ({ solution, onClose, isFromTicket, onUse }) => {
  ...
  const handleTicketClick = async (ticketId) => {
    try {
      ...
      const response = await fetch(
        `${config.API_BASE_URL}/ticket-data/${ticketId}`
      );
      if (!response.ok) {
  ...
  const submitRatingToServer = async (solutionId, rating) => {
    ...
    const response = await fetch(
      `${config.API_BASE_URL}/solution-data/${solutionId}/rate`,
      {
  ...
  useEffect(() => {
    async function fetchMedia() {
      try {
        const response = await fetch(
          `${config.API_BASE_URL}/media/solution/${solution.solution_id}`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
    ...
    async function addView() {
      try {
        const response = await fetch(
          `${config.API_BASE_URL}/solution-data/${solution.solution_id}/view`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
    ...
    const fetchRelatedTickets = async () => {
      try {
        const response = await fetch(
          `${config.API_BASE_URL}/ticket-solution/solution/${solution.solution_id}`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-container">
        ...
        <SolutionDataTable
          tabledata={solutionDataForTable}
          isDetailView={true}
        />
        ...
        >
          ...
          <UserRating
            initialRating={solution.userRating}
          ...
          <div className="steps-grid">
            {media.map((imageItem, index) => (
              <div key={index} className="step-container">
                ...
                />
                <p className="step-comment">{imageItem.comment}</p>{" "}
                {/* Comment below image */}
              </div>
            ))}
          </div>
